...
function setApiDefaults(
  ...
  vi.mocked(tauriApi.getSkillDetails).mockImplementation(async (skillKey) => {
    const details = detailsBySkillKey[skillKey]
    if (!details) {
      throw new Error(`missing details for ${skillKey}`)
    }
    return details
  })
}
...
describe('App critical actions', () => {
  it('uses independent desktop scroll containers for left and right columns', async () => {
    ...
    const skillsCard = screen.getByRole('heading', { name: 'Skills' }).closest('.rounded-lg')
    ...
    const skillsList = within(skillsCard as HTMLElement).getByRole('list')
    ...
    const workspaceLabel = within(detailsCard as HTMLElement).getByText('Workspace')
  ...
  it('loads initial state and selected skill details', async () => {
    ...
    expect(tauriApi.getSkillDetails).toHaveBeenCalledWith(projectSkill.skill_key)
  })
